...
private:
    ...
    void insert_fixup(Node<K, V> *z) {
        // TODO
    	size_ = internal_node_count() + leaf_count();
    ...
    int diameter(Node<K, V> *node) const {
    	...
    	return std::max(std::max(diameter(node->right),diameter(node->left)),((height(node->right)+1)+(height(node->left)+1)+1));
    }
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
    	...
    	return (level * width(root_, level)) + sum_levels(root_, level + 1);
    }
    ...
    size_t sum_null_levels(Node<K, V> *node, size_t level) const {
    	...
    	if (level == h) {
    		return level * (2 * width(root_, level - 1));
    	}
    	if (level == 0) {
    		if (root_ == nullptr) {
    			return 1;
    		} else {
    			return sum_null_levels(root_, level + 1);
    		}
    	}
    	return (level * ((2 * width(root_, level - 1)) - width(root_, level))) + sum_null_levels(root_, level + 1);
    }
};
